...
// Consulta de todos los hamsters
$sql = "SELECT * FROM mascotas WHERE tipo='Hamster' AND estado='disponible'";

$resultado = mysqli_query($conexion, $sql);
$cantidad_hamsters = mysqli_num_rows($resultado);
?>
...
<head>
<meta charset="UTF-8">
<title>Hamsters en adopción</title>
...
.modal-content { background: white; margin: 50px auto; padding: 20px; border-radius: 15px; width: 90%; max-width: 500px; text-align: center; max-height: 80vh; overflow-y: auto; }
...
<div class="header">
  ...
  <div class="contador"> Amigos pequeñitos 🐹✨: <?= $cantidad_hamsters ?></div>
</div>

<h1>🐾 Nuestros hamsters esperan un hogar 🐹💖</h1>

<div class="contenedor">
<?php if ($cantidad_hamsters > 0): ?>
  ...
  <?php endwhile; ?>
<?php else: ?>
  <p>No hay hamsters disponibles por ahora 🐹💔</p>
<?php endif; ?>
</div>
...
<div id="bienvenidaModal" style="position:fixed;top:0;left:0;width:100%;height:100%;
  ...
  <div style="background:#fff0f6;padding:25px 35px;border-radius:16px;text-align:center;
    ...
    <p style="color:#555;margin-bottom:20px;">Nuestros hamsters esperan un hogar lleno de amor 💕</p>
  ...
  document.querySelectorAll(".btn-adoptar").forEach(function(btn) {
    btn.addEventListener("click", function() {
      ...
      Swal.fire({
        ...
        text: 'Para poder adoptar este hamster, primero debes registrarte.',
        icon: 'info',
