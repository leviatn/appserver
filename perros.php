...
// Consulta de todos los perros
$sql = "SELECT * FROM mascotas WHERE tipo='Perro' AND estado='disponible'";
$resultado = mysqli_query($conexion, $sql);
$cantidad_perros = mysqli_num_rows($resultado);
?>
...
.contador { 
    background: #ff914d; /* mismo naranja de los botones */
    color: white; 
    padding: 8px 12px; 
    border-radius: 10px; 
    font-weight: bold; 
...
<div class="header">
  ...
  <div class="contador"> Amigos perrunos 🐾🐶: <?= $cantidad_perros ?></div>
</div>

<h1>🐾 Nuestros perritos esperan un hogar 🐶</h1>

<div class="contenedor">
<?php if ($cantidad_perros > 0): ?>
  ...
  <?php endwhile; ?>
<?php else: ?>
  <p>No hay perritos disponibles por ahora 🐕💔</p>
<?php endif; ?>
</div>
...
<div id="bienvenidaModal" style="position:fixed;top:0;left:0;width:100%;height:100%;
  ...
  <div style="background:#fff0f6;padding:25px 35px;border-radius:16px;text-align:center;
    ...
    <p style="color:#555;margin-bottom:20px;">Nuestros perritos esperan un hogar lleno de amor 💕</p>
    <button onclick="cerrarModalBienvenida()" 
            style="padding:8px 16px;border:none;border-radius:8px;
                   background:#a3d8f4;color:#000;cursor:pointer;">
      ¡Vamos!
    </button>
  </div>
</div>
  ...
  document.querySelectorAll(".btn-adoptar").forEach(function(btn) {
    btn.addEventListener("click", function() {
      ...
      Swal.fire({
        ...
        text: 'Para poder adoptar este perrito, primero debes registrarte.',
        icon: 'info',
