...
// Consulta de todas las aves
$sql = "SELECT * FROM mascotas WHERE tipo='Ave' AND estado='disponible'";
$resultado = mysqli_query($conexion, $sql);
$cantidad_aves = mysqli_num_rows($resultado);
?>
...
<div class="header">
  ...
  <div class="contador"> Amigos alados 🐦✨: <?= $cantidad_aves ?></div>
</div>

<h1>🐾 Nuestras aves esperan un hogar 🐦‍🔥</h1>

<div class="contenedor">
<?php if ($cantidad_aves > 0): ?>
  ...
  <?php endwhile; ?>
<?php else: ?>
  <p>No hay aves disponibles por ahora 🐦💔</p>
<?php endif; ?>
</div>
...
<div id="bienvenidaModal" style="position:fixed;top:0;left:0;width:100%;height:100%;
  ...
  <div style="background:#fff0f6;padding:25px 35px;border-radius:16px;text-align:center;
    ...
    <p style="color:#555;margin-bottom:20px;">Nuestras aves esperan un hogar lleno de amor 💕</p>
  ...
  document.querySelectorAll(".btn-adoptar").forEach(function(btn) {
    btn.addEventListener("click", function() {
      ...
      Swal.fire({
        ...
        text: 'Para poder adoptar esta ave, primero debes registrarte.',
        icon: 'info',
